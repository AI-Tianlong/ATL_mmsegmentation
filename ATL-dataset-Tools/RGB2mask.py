...
import numpy as np
from ATL_path import find_data_list, mkdir_or_exist, scandir
...
classes = [
    'cluster', 'Building', 'di_ai_zhi_bei', 'Tree', 'car', 'bu_tou_shui_mian'
]

platte = [[255, 0, 0], [0, 0, 255], [0, 255, 255], [0, 255, 0], [255, 255, 0],
          [255, 255, 255]]
...
for RGB_label_path in tqdm(RGB_label_lists):
    RGB_label = np.array(Image.open(RGB_label_path)).astype(
        np.uint8)  # 6000*6000*3
    #把一张图展开，6000*6000*3 --> 36000000*3
    #然后36000000*3 * 3*1 = 36000000*1
    #然后，去platte_idx里找到对应的类别
    RGB_idx = np.matmul(
        RGB_label.reshape(-1, RGB_label.shape[2]),
    ...
    Image.fromarray(
        mask_label.reshape(RGB_label.shape[0], RGB_label.shape[1])).save(
            os.path.join(
                MASK_path,
                os.path.basename(RGB_label_path).replace('.tif', '.png')))
