        ...
        for i in range(h//crop_size):
            for j in range(w//crop_size):
                ...
                if img_or_label == 'img_8bit_NirRGB':
                    ...
                    new_512 = np.zeros((crop_size,crop_size, c),dtype=np.uint16)
                    new_img = Driver.Create(out_path, crop_size, crop_size, c, gdal.GDT_UInt16)
...
def main():
    ...
    if args.out_dir is None:
        out_dir = os.path.join(args.dataset_path, 'data', 'five_billion_pixels')
    else:   
